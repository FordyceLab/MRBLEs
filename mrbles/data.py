...
class TableDataFrame:
    ...
    def combine(self, data):
        ...
        """
        if isinstance(self.data, pd.DataFrame) & isinstance(data, pd.DataFrame):
            index = data.index
            self._dataframe = pd.concat(
                [data.reset_index(drop=True), self.data.reset_index(drop=True)], axis=1, sort=False
            )
    ...
    def _add_info(info_data, dataframe, codes=None, prefix="info."):
        if isinstance(info_data, pd.DataFrame):
            ...
            if (codes is None) and ("code" in dataframe.columns):
                ...
                for code in codes:
                    dataframe.loc[dataframe.code == code, column_names] = info_data_prefix.iloc[
                        int(code)
                    ].values
            else:
                for code in codes:
                    dataframe.loc[code, column_names] = info_data_prefix.iloc[int(code)]
        else:
            dataframe.loc[code, "set.sequence"] = info_data[code]
        return dataframe
...
class ImageDataFrame:
    ...
    def __getitem__(self, index=None):
        ...
        if isinstance(index, string_types):
            ...
            data = {key: data.loc[index[1:]] for key, data in self.data.items()}
    ...
    def combine(self, images):
        ...
        if isinstance(self.data, dict) & isinstance(images, dict):
            self._dataframe = {
                key: self.data[key].combine_first(images[key]) for key in self.data.keys()
            }
        elif isinstance(self.data, xr.DataArray) & isinstance(images, xr.DataArray):
            self._dataframe = self.data.combine_first(images)
        else:
            raise ValueError("Not dict or Xarray DataArray: %s." % type(images))
    ...
    def shift_channel(self, channel, x_shift, y_shift):
        ...
        if isinstance(data_shift, dict):
            ...
            data_shift.loc[:, channel] = data_shift.loc[:, channel].shift(x=y_shift, y=y_shift)
        else:
            data_shift = None
    ...
    def _check_crop(self, data):
        if isinstance(data, dict):
            data_crop = {
                key: value.loc[dict(x=self._crop_x, y=self._crop_y)] for key, value in data.items()
            }
...
class ImageSetRead(ImageDataFrame):
    ...
    def __init__(self, file_path, series=0, output="xr"):
        ...
        super().__init__()
        self._dataframe, self._metadata, self._files = self.load(file_path, series)
        self.output = output
    ...
    def load(cls, file_path, series=0):
        ...
        image_data = cls._convert_to_xd(panel_data, image_metadata, file_path, series)
    ...
    def t_deltas(self):
        ...
        t_deltas = [
            float(xm.attributes["DeltaT"].value) for xm in xml_tree.getElementsByTagName("Plane")
        ]
        return t_deltas
    ...
    def scan_paths(cls, paths, pattern=".*.tif"):
        ...
        if isinstance(paths, str):
            ...
            image_files = [cls.scan_path(path, pattern=pattern) for path in paths]
        else:
    ...
    def _convert_to_xd(data, metadata, file_path, series):
        ...
        if data.ndim == 2:
            ...
            panel_data = xr.DataArray(data, dims=dims, coords={"c": channels})
        return panel_data
