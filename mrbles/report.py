        ...
        for dim_idx, dim in enumerate(dims):
            if ring is not None:
                if type(ring) is int:
                    ...
                    for dim_r in ring:
                        cv2.circle(
                            img, (int(dim_r[0]), int(dim_r[1])), int(ceil(dim_r[2])), (0, 255, 0), 1
                        )
...
class ClusterCheck(TableDataFrame):
    ...
    def plot_3D(self, min_prob=None):
        ...
        colors = np.multiply(bead_set.code.values.astype(int), np.ceil(255 / len(target)))
        dims_pre_icp = [
            dim
            for dim in bead_set.columns
            if ("_ratio.mask_inside" in dim) and (dim[len(dim) - 18 :] == "_ratio.mask_inside")
        ]
        dims_post_icp = [dim for dim in bead_set.columns if "_ratio.mask_inside_icp" in dim]
        dims_names = [dim.replace(".mask_inside", "") for dim in dims_pre_icp]
        if len(dims_names) > 3:
            warnings.warn(
                "Set has more than 3 dimensions, only first 3 dimensions are used for 3D plot.",
                UserWarning,
            )
        if len(dims_names) < 3:
        ...
        bead_ratios_pre = go.Scatter3d(
            ...
            mode="markers",
            marker=dict(
                size=3, color=colors, colorscale="Rainbow", opacity=0.6, symbol="circle-open"
            ),
        )
        ...
        bead_ratios_post = go.Scatter3d(
            ...
            text=bead_set["code"].values + 1,
            mode="markers",
            marker=dict(size=3, color=colors, colorscale="Rainbow", opacity=0.6),
        )
        ...
        mean_ratios = go.Scatter3d(
            ...
            text=list(range(1, len(target) + 1)),
            mode="markers",
            marker=dict(size=4, color="red", opacity=0.5, symbol="diamond"),
        )
        ...
        layout = go.Layout(
            showlegend=True,
            scene=dict(
                xaxis=dict(title=dims_names[0]),
                yaxis=dict(title=dims_names[1]),
                zaxis=dict(title=dims_names[2]),
            ),
    ...
    def plot_2D(self, colors, ci_trace=None, min_prob=None):
        ...
        color_exclude = [color for color in self._decode._target.columns if color not in colors][0]
        ...
        colors_data = np.multiply(mrbles_data.code.values, (255 / len(target_ratios))).astype(int)
        codes_found = np.unique(mrbles_data.code)
        color_pos = [np.argwhere(self._decode._target.columns == colors[0])[0, 0]]
        color_pos.append(np.argwhere(self._decode._target.columns == colors[1])[0, 0])
        ...
        bead_ratios_plot = go.Scatter(
            ...
            mode="markers",
            marker=dict(size=3, color=colors_data, colorscale="Rainbow", opacity=0.6),
        )
...
class BeadsReport(object):
    ...
    def __init__(self, data, images, masks, assay_channel, codes=None, files=None, sort=True):
        # Speed settings
        ...
        if codes is not None:
            if isinstance(codes, list):
                self._dataframe = self._dataframe[self._dataframe["code"].isin(codes)]
            else:
                self._dataframe = self._dataframe[self._dataframe["code"] == codes]
        if files is not None:
            if isinstance(files, list):
                self._dataframe = self._dataframe[self._dataframe["f"].isin(files)]
            else:
                self._dataframe = self._dataframe[self._dataframe["f"] == files]
        self._time_estimate()
    ...
    def _time_estimate(self):
        ...
        print("Total estimated time required: %i minutes" % round(total_time))
    ...
    def _per_bead_plot(self, idx, image, g_idx, dim, bead_num, img_num):
        ax_sub = plt.subplot2grid((bead_num, img_num), (g_idx, idx))
        if g_idx == 0:
        ...
        if idx == 0:
            ...
            ax_sub.set_xlabel(
                "I: %i" % (dim[self.assay_channel + "." + str(image.c.values)]), size=3
            )
            ...
            ax_sub.set_xlabel(
                "R: %0.3f" % (dim[str(image.c.values) + "_ratio" + "." + self.npl_mask]), size=3
            )
            ax_sub.imshow(image.astype(np.int), vmin=0, vmax=self.max_npl)
        ax_sub.set_yticklabels([])
        ax_sub.set_xticklabels([])
        ax_sub.tick_params(axis="both", which="both", length=0)
    ...
    def _iter_dims(self, index, dim, figs, bead_num, img_num):
        ...
        [
            ...
            for idx, image in enumerate(
                figs[d_f, :, slice(int(y_min), int(y_max)), slice(int(x_min), int(x_max))]
            )
        ]
    ...
    def _per_pdf(self, filename, dim_step=33):
        with PdfPages(filename) as pdf_object:
            [
                ...
                else self._per_set_pdf(
                    self._dataframe[x : self.beads_num], self._images, pdf_object
                )
...
class QCReport(object):
    ...
    def generate(self, filename, savefig=False):
        ...
        with PdfPages(filename) as pdf_object:
            self._add_figure([self.bead_size], pdf_object)
            if "code" in self._per_bead_data.columns:
                self._add_figure([self.beads_per_code], pdf_object)
    ...
    def bead_size(self):
        ...
        self._per_bead_data[d_name].plot(
            ...
            bins=100,
            color="lightgray",
            title="Beads Diameter (%s) Mean: %0.2f SD: %0.2f" % (dim, b_mean, b_std),
        ).set_xlim(left=x_left, right=x_right)
        self._per_bead_data[d_name].plot(
    ...
    def beads_per_code(self):
        ...
        self._per_bead_data.groupby(["set", "code"]).size().plot(
            kind="hist",
            color="lightgray",
            title="Beads-per-code (N) Mean: %0.2f SD: %0.2f" % (b_mean, b_std),
        )
        self._per_bead_data.groupby(["set", "code"]).size().plot(
            kind="kde", secondary_y=True, color="black", alpha=0.7
        )
    ...
    def npl_plots(self, pdf_object):
        ...
        g.map(
            sns.regplot,
            "Sm_ratio.mask_inside",
            "Tm_ratio.mask_inside",
            fit_reg=False,
            scatter=True,
            scatter_kws={"alpha": 0.3, "color": "darkgray"},
        ...
        # After CI filter
        g = sns.FacetGrid(
            self._per_bead_data.query("prob > 0.95"), col="info.Dy", col_wrap=3, sharey=True
        )
        ...
        if self._savefig is True:
            plt.savefig(
                (self.report_folder + "Sm_vs_Tm_ratios_pre-ICP_CI95_filter.png"), dpi=self.dpi
            )
        plt.close()

        # Post ICP
        sns.distplot(
            self._per_bead_data["Dy_ratio.mask_inside_icp"], hist=True, kde=False, bins=1000
        )
        ...
        g.map(
            sns.regplot,
            "Sm_ratio.mask_inside_icp",
            "Tm_ratio.mask_inside_icp",
            fit_reg=False,
            scatter=True,
            scatter_kws={"alpha": 0.3, "color": "darkgray"},
            line_kws={"color": "black"},
        ...
        if self._savefig is True:
            plt.savefig(
                (self.report_folder + "Sm_vs_Tm_ratios_post-ICP_CI95_filter.png"), dpi=self.dpi
            )
        plt.close()

    def assay_contamination(self, pdf_object):
        self._per_bead_data.plot(
            kind="scatter",
            figsize=(9, 6),
    ...
    def npl_covar_plots(self):
        ...
        for X1, y1 in zip(Dy_masks_means, Dy_masks_sds):
            ...
            plt.annotate(
                "Slope: %0.3f Intercept: %0.3f PR2: %0.2f" % (slope, intercept, r2),
                xy=(X[1], max(y)),
            )
            n += 1
    ...
    def code_stability(self):
        ...
        g = sns.FacetGrid(
            data_per_step_all.sort_values("code"), col="code", col_wrap=6, sharey=False
        )
...
class GenerateCodes(object):
    ...
    def __init__(self, colors, s0s, slopes, nsigma):
        if not len(colors) == len(slopes) == len(s0s):
            raise ValueError("Length colors, nsigmas and slopes not equal: %s." % sys.exit()[1])
        self._colors = colors
        self._s0s = s0s
        self._slopes = slopes
        self._nsigma = nsigma
    ...
    def _levels(self, nsigma=None):
        ...
        for idx, _ in enumerate(self._colors):
            levels.append(self.get_levels(self._s0s[idx], self._slopes[idx], nsigma))
        return levels
    ...
    # Experimental
    def to_csv_rep(self, filename, repeats, labels=None, pos=True):
